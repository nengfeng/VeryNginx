...
describe("Promotion evaluate (observe-only)", function()
    ...
    it("full lifecycle: transition and remove", function()
        ...
        sm.transition("203.0.113.9", "promoted", { list = "scanner_drop" })
        local e = sm.get("203.0.113.9")
        assert.are.equal("promoted", e.state)
        assert.are.equal("scanner_drop", e.list)
        sm.transition("203.0.113.9", "installed", { installed_at = ngx.time() })
        e = sm.get("203.0.113.9")
        assert.are.equal("installed", e.state)
        sm.remove("203.0.113.9")
        assert.is_nil(sm.get("203.0.113.9"))
    end)
    ...
    it("scope_validation_pending transitions", function()
        ...
        sm.to_scope_validation_pending("203.0.113.50")
        ...
        sm.from_scope_validation_pending("203.0.113.50", true)
        e = sm.get("203.0.113.50")
        assert.are.equal("installed", e.state)
    ...
    it("scope_validation_pending → degraded when entry missing", function()
        ...
        sm.to_scope_validation_pending("203.0.113.51")
        -- Helper restored but entry was NOT verified (e.g., nft rules were lost)
        sm.from_scope_validation_pending("203.0.113.51", false)
        local e = sm.get("203.0.113.51")
