...
MMDB_lookup_result_s MMDB_lookup_sockaddr(const MMDB_s *const mmdb, const struct sockaddr *const sockaddr, int *const mmdb_error);
int MMDB_read_node(const MMDB_s *const mmdb, uint32_t node_number, MMDB_search_node_s *const node);
...
-- error codes
-- https://github.com/maxmind/libmaxminddb/blob/1.12.2/include/maxminddb.h#L60-L71
...
-- data type
-- https://github.com/maxmind/libmaxminddb/blob/1.12.2/include/maxminddb.h#L33-L48
...
function _M.init(profiles)
  ...
  for profile, location in pairs(profiles) do
    ...
    ngx.log(ngx.INFO, "init maxminddb profile: ", profile, " location: ", location, " init result: ",
...
function _M.lookup(ip, lookup_path, profile)
  ...
  if mmdb_error[0] ~= MMDB_SUCCESS then
    return nil, 'lookup failed: ' .. mmdb_strerror(maxm, mmdb_error[0])
  end

  if gai_error[0] ~= MMDB_SUCCESS then
    return nil, 'lookup failed: ' .. gai_strerror(maxm, gai_error[0])
  end
  ...
  if status ~= MMDB_SUCCESS then
    return nil, 'get entry data failed: ' .. mmdb_strerror(maxm, status)
  end
  ...
  if status ~= MMDB_SUCCESS then
    return nil, 'dump entry data failed: ' .. mmdb_strerror(maxm, status)
  end
